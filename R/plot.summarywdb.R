plot.summarywdb <- function(x, col=NULL, ...)
{
        ...
        mtext("Sensitivity", 3, line=3, adj=1, col=col[1])
        mtext("Positive predicted value", 3, line=2, adj=1, col=col[2])
        mtext("Specificity", 3, line=1, adj=1, col=col[3])
        mtext( "Classes", 1, line=2)
        mtext( "Percentage", 2, line=2)
}
